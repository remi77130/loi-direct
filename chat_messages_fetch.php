...
$chk->bind_param('i',$room_id); $chk->execute();
$room = $chk->get_result()->fetch_assoc(); $chk->close();
if(!$room){ http_response_code(404); echo json_encode(['ok'=>false,'error'=>'room']); exit; }
if ((int)$room['is_private']===1){ http_response_code(403); echo json_encode(['ok'=>false,'error'=>'private']); exit; }

if($after>0){
  $sql = "SELECT m.id,m.body,m.created_at,u.pseudo AS sender
          FROM chat_messages m JOIN users u ON u.id=m.sender_id
          WHERE m.room_id=? AND m.id>? ORDER BY m.id ASC LIMIT 200";
  $stmt=$mysqli->prepare($sql); $stmt->bind_param('ii',$room_id,$after);
}else{
  $sql = "SELECT m.id,m.body,m.created_at,u.pseudo AS sender
          FROM chat_messages m JOIN users u ON u.id=m.sender_id
          WHERE m.room_id=? ORDER BY m.id DESC LIMIT 50";
  $stmt=$mysqli->prepare($sql); $stmt->bind_param('i',$room_id);
}
