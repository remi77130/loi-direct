...
if (mb_strlen($pass1) < 8 || mb_strlen($pass1) > 128) {
  $errors[] = "Mot de passe: 8–128 caractères.";
}
if ($pass1 !== $pass2) {
...
if (!$errors) {
  ...
  $ins = $mysqli->prepare('INSERT INTO users (pseudo, password_hash) VALUES (?, ?)');
  if (!$ins) { $errors[]="Erreur serveur"; error_log("register prepare: ".$mysqli->error); }
  else {
    $ins->bind_param('ss', $pseudo, $hash);
    if ($ins->execute()) {
      session_regenerate_id(true);
      ...
      header('Location: '.rtrim(APP_BASE,'/').'/index.php', true, 303);
      exit;
    } else { $errors[]="Erreur serveur"; error_log("register exec: ".$ins->error); }
    $ins->close();
  }
}
...
<input type="password" id="password" name="password" minlength="8" maxlength="20"  required placeholder="••••••••">
