 ...
 * - Affiche un projet "published" (titre/summary/corps)
 * - Gère like/unlike via fetch() (AJAX)
 * - Liste/ajout/suppression des commentaires
 * - Affiche la galerie d’images (vignettes → original)
 * - Redirection 301 vers l’URL canonique /p/{id}-{slug}
 */

declare(strict_types=1);
session_start();

require __DIR__ . '/config.php'; // constantes (APP_BASE, UPLOAD_URL, helpers: slugify(), tag_url(), …)
require __DIR__ . '/db.php';     // $mysqli (connexion MySQLi)
...
/**
 * $base = base path du script (ex: /loi). On l’utilise pour fabriquer
 * des URL relatives robustes, sans supposer la racine serveur.
 */
$base = rtrim(dirname($_SERVER['PHP_SELF']), '/\\');

/* ----------------------------------------------------------------------
 * Entrée : id de projet (numérique). On force l’int pour éviter les surprises.
 * -------------------------------------------------------------------- */
$id = (int)($_GET['id'] ?? 0);

$project    = null;   // ligne du projet (array assoc) ou null si non trouvé
$likesCount = 0;      // nombre total de likes
$likedByMe  = false;  // est-ce que l’utilisateur courant a liké ce projet ?

/* ----------------------------------------------------------------------
 * Chargement du projet s’il y a un id > 0
 * -------------------------------------------------------------------- */
if ($id > 0) {

  // On récupère le projet publié + l’auteur (pseudo)
  $sql = 'SELECT
            p.id, p.title, p.summary, p.body_markdown, p.published_at,
            p.author_id, u.pseudo AS author
          FROM law_projects p
  ...
  /* ------------------------------------------------------------------
   * Redirection 301 vers l’URL canonique /p/{id}-{slug}
   * Objectif : SEO & URLs propres. Si on charge via /project.php?id=…,
   * on redirige pour avoir /p/123-mon-titre
   * ---------------------------------------------------------------- */
  if ($project) {
    $slug     = slugify($project['title']);
    $expected = $base . '/p/' . (int)$id . '-' . $slug;               // chemin attendu
    $path     = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH) ?? ''; // chemin courant
  ...
  /* ------------------------------------------------------------------
   * Récupération des statistiques de likes + état "liké par moi"
   * ---------------------------------------------------------------- */
  if ($project) {
    // total likes
  ...
  /* ------------------------------------------------------------------
   * Galerie : toutes les images du projet (vignettes + original)
   * On ne requête que si le projet existe.
   * ---------------------------------------------------------------- */
  $images = [];
  if ($project) {
    $st = $mysqli->prepare(
...
<!-- Canonical SEO : on renvoie vers l’URL canonique même depuis project.php -->
<?php $slugForCanonical = ($project ? slugify($project['title']) : ''); ?>
<link rel="canonical" href="<?php echo APP_BASE; ?>/p/<?php
  echo (int)$id . '-' . htmlspecialchars($slugForCanonical, ENT_QUOTES); ?>">
...
<body>
  ...
  • <?= htmlspecialchars($project['published_at'] ? date('d/m/Y H:i', strtotime($project['published_at'])) : '', ENT_QUOTES); ?>
        ...
        <div class="toolbar">
          ...
          <!-- Si l’auteur courant == auteur du projet → bouton Supprimer -->
        ...
        <?php if ($images): ?>
          <!-- Galerie d’images : on affiche la miniature si dispo, sinon l’original.
               Le clic ouvre l’original dans un nouvel onglet. -->
          <hr style="border:0;border-top:1px solid #334155;margin:16px 0">
          <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:10px">
            <?php foreach ($images as $im):
              ...
              <a href="<?= APP_BASE ?>/uploads/<?= htmlspecialchars($im['path'],ENT_QUOTES) ?>"
                ...
                <img
                  src="<?= APP_BASE ?>/uploads/<?= htmlspecialchars($thumb,ENT_QUOTES) ?>"
        ...
        <?php
        // Liste des tags du projet (chips)
        $tags = [];
        $tg = $mysqli->prepare(
         ...
         * LEGACY / BLOC MORT
         * ------------------
         * Le bloc suivant faisait une 2e galerie basée sur $pics,
         * mais $pics n’est plus alimenté. On le garde commenté pour mémo.
...
<?php if ($project): ?>
  ...
  <section id="comments" class="card" style="margin-top:14px">
    ...
    <!-- Flash erreurs (ex: validation commentaire) -->
...
<?php endif; ?>
  ...
  <!-- Toast pour feedback de copie -->
  <div id="toast" class="toast">Lien copié ✅</div>

<script>
/**
 * Like / Unlike
 * - Appelle like_toggle.php en POST (id + csrf)
...
if (likeBtn) {
  likeBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      // noop (on pourrait afficher un toast d’erreur ici)
    }
...
/**
 * Partage (copie le lien courant dans le presse-papiers)
 * - Utilise Clipboard API si dispo, sinon fallback sélection/execCommand.
 */
