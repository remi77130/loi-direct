...
$types  = '';
$params = [];
if ($mine) { $types .= 'i'; $params[] = $user_id; }
if ($q !== '') {
  $like = '%'.$q.'%';
...
/* --- Pagination (mêmes filtres, sans LIMIT/OFFSET) --- */
$countSql = "SELECT COUNT(*) FROM law_projects p WHERE $where";
$countTypes  = '';
$countParams = [];
if ($mine) { $countTypes .= 'i'; $countParams[] = $user_id; }
if ($q !== '') {
  $countTypes .= 'sss'; $like = '%'.$q.'%'; array_push($countParams, $like, $like, $like);
}
...
<?php $slug = slugify($p['title']); // Dans la boucle des cartes, ON utilise le slug?>
<div style="margin-top:10px">
  <a class="btn" href="<?= APP_BASE ?>/p/<?= (int)$p['id'] ?>-<?= htmlspecialchars($slug, ENT_QUOTES) ?>">Lire</a>
  <span style="margin-left:8px;font-size:12px;color:#94a3b8">❤ <?= (int)$p['likes_count'] ?></span>
</div>
