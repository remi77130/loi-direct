...
<?php
$pics = [];
$st = $mysqli->prepare('SELECT path, thumb_path, original_name FROM project_images WHERE project_id=? ORDER BY id');
$st->bind_param('i', $id);
$st->execute();
$r = $st->get_result();
$pics = $r->fetch_all(MYSQLI_ASSOC);
$st->close();
?>
...
<?php if (!empty($pics)): ?>
  <div style="display:flex; gap:10px; flex-wrap:wrap; margin-top:12px">
    <?php foreach ($pics as $img): ?>
      <a href="<?= UPLOAD_URL . '/' . htmlspecialchars($img['path'], ENT_QUOTES) ?>" target="_blank" rel="noopener">
        <img
          src="<?= UPLOAD_URL . '/' . htmlspecialchars($img['path'], ENT_QUOTES) ?>"
          alt=""
