...
$stmt = $mysqli->prepare('SELECT id, pseudo, sex, height_cm, postal_code, city FROM users WHERE id=? LIMIT 1');
...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  if (empty($csrf) || !hash_equals($_SESSION['csrf'], $csrf)) {
    ...
    // Inputs
    $sex_in    = (string)($_POST['sex'] ?? '');
    $height_in = (string)($_POST['height_cm'] ?? '');
    $postal_in = trim((string)($_POST['postal_code'] ?? ''));
    $city_in   = trim((string)($_POST['city'] ?? ''));

    // Sexe
    $sex_new = null;
    if ($sex_in !== '') {
      $sex_in = strtolower($sex_in);
    ...
    $postal_new = '';
    if ($postal_in !== '') {
      if (preg_match('/^\d{5}$/', $postal_in)) $postal_new = $postal_in;
      else $errors[] = 'Code postal invalide.';
    }

    // Ville (optionnelle, mais si fournie avec CP, on vérifie la cohérence)
    $city_new = '';
    if ($city_in !== '') $city_new = mb_substr($city_in, 0, 255);

    if ($postal_new !== '' && $city_new !== '') {
      $v = $mysqli->prepare("SELECT 1 FROM cities WHERE postal_code=? AND city=? LIMIT 1");
      if ($v) {
        $v->bind_param('ss', $postal_new, $city_new);
        $v->execute(); $v->store_result();
        if ($v->num_rows===0) $errors[] = 'Ville non trouvée pour ce code postal.';
        $v->close();
      }
    }
    ...
    if (!$errors){
      ...
      if (!$st) { $errors[]='Erreur serveur.'; }
        ...
        // NULL pour height si non renseigné
        $height_param = $height_new ?? null;
        $st->bind_param('sissi', $sex_new, $height_param, $postal_new, $city_new, $uid);
        if ($st->execute()){
    ...
    // Réinjecter valeurs saisies en cas d’erreurs
    $sex    = $sex_new;
    $height = $height_new;
    $postal = $postal_new;
    $city   = $city_new;
...
<body>
  <div class="card">
    ...
    <form method="post" novalidate>
      ...
      <input id="pc" name="postal_code" pattern="\d{5}" maxlength="5" value="<?= htmlspecialchars($postal,ENT_QUOTES) ?>">

      <label for="city">Ville</label>
      <input list="cities" id="city" name="city" autocomplete="off" value="<?= htmlspecialchars($city,ENT_QUOTES) ?>">
...
<script>
const BASE='<?= APP_BASE ?>';
const pc=document.getElementById('pc'), dl=document.getElementById('cities');
pc.addEventListener('input', async ()=>{
  const v=pc.value.trim();
  dl.innerHTML='';
  if(/^\d{5}$/.test(v)){
    try{
      const r=await fetch(`${BASE}/city_lookup.php?postal=${encodeURIComponent(v)}`,{cache:'no-store'});
      const j=await r.json();
      if(j.ok){ j.cities.forEach(c=>{ const o=document.createElement('option'); o.value=c; dl.appendChild(o); }); }
    }catch(_){}
  }
});
</script>
