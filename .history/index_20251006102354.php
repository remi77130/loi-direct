...
async function openUserModal(userId){
  try {
    const r = await fetch(`${BASE}/user_card.php?id=${encodeURIComponent(userId)}`, {credentials:'same-origin'});
    ...
    `;
    umProfile.href = `${BASE}/profile.php?id=${userId}`;
    umMsgForm.querySelector('input[name="recipient_id"]').value = userId;

    showModal();
  } catch(e){ console.error(e); alert('Impossible d’ouvrir la fiche utilisateur.'); }
}

// Ouvrir depuis bouton "Message" OU depuis le pseudo cliquable
document.addEventListener('click', (ev) => {
  const btn = ev.target.closest('.js-open-user, .user-link[data-user-id]');
  if (!btn) return;
  ev.preventDefault();
