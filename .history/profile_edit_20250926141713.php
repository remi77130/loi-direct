...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  if (empty($csrf) || !hash_equals($_SESSION['csrf'],$csrf)) {
    ...
    if (!$errors) {
      $stmt = $mysqli->prepare('UPDATE users SET sex=?, height_cm=? WHERE id=?');
      if (!$stmt) { error_log('profile_edit prepare: '.$mysqli->error); $errors[]='Erreur serveur.'; }
      else {
        // bind: ssi mais height peut être NULL -> utilise bind_param dyn + null-safe
        // astuce: "i" accepte null via ->bind_param et set to null avec $height_new = null
        $stmt->bind_param('sii', $sex_new, $height_new, $uid);
        // si $sex_new est null, forcer type null :
        if ($sex_new===null) $sex_new = null; // deja null
        if ($height_new===null) $height_new = null;

        if (!$stmt->execute()) {
          error_log('profile_edit exec: '.$stmt->error);
          $errors[]='Erreur serveur.';
        }
        $stmt->close();

        if (!$errors) {
          header('Location: '.rtrim(APP_BASE,'/').'/profile.php?id='.$uid, true, 303);
          exit;
        }
      }
    }
