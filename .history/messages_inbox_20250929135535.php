...
// en calculant l'interlocuteur (other_id) côté SQL.
$sql = '
  SELECT
...
document.addEventListener('submit', async (e)=>{
  ...
  try{
    ...
    // exiger texte OU image côté client
...
// --- Après form.before(out); ---
const card = form.closest('.msg-card');
const head = card.querySelector('.msg-head');

// Aperçu (dernier message)
const previewDiv = card.querySelector('.msg-preview') || card.insertBefore(document.createElement('div'), card.querySelector('.msg-body'));
...
// Remplace la date dans l’en-tête (garde la flèche)
head.innerHTML = head.innerHTML.replace(/— .*?<span class="chev">/, `— ${dateStr} <span class="chev">`);

// Remonter cette card en haut de la liste (même parent)
const listContainer = card.parentElement;
const firstCard = listContainer.querySelector('.msg-card');
if (firstCard && card !== firstCard) {
  listContainer.insertBefore(card, firstCard);
}
