...
$stmt = $mysqli->prepare('SELECT id, pseudo, sex, height_cm, situation FROM users WHERE id=? LIMIT 1');
...
$situation_in = trim((string)($_POST['situation'] ?? ''));
    ...
    if (!$errors){
      $st = $mysqli->prepare("UPDATE users SET sex=?, height_cm=? WHERE id=?");
      if (!$st) { $errors[]='Erreur serveur.'; }
      else {
        // NULL pour height si non renseigné
        $height_param = $height_new ?? null;
        $st->bind_param('sis', $sex_new, $height_param, $uid);
        if ($st->execute()){
  ...
  <div class="card">
    ...
    <form method="post" novalidate>
      ...
      <select id="situation" name="situation">
        <option value="" <?= $situation===null?'selected':'' ?>>— Non renseigné —</option>
        <option value="Célibataire" <?= $situation==='Celibataire'?'selected':'' ?>>Célibataire</option>
        <option value="En couple" <?= $situation==='En couple'?'selected':'' ?>>En coupleme</option>
      </select>
