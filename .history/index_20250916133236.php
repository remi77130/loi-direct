...
<header>
  ...
  <form method="get" action="<?= APP_BASE ?>/index.php" style="display:flex;gap:8px;align-items:center">
    ...
    <?php if ($tagSlug !== ''): ?><input type="hidden" name="tag" value="<?= htmlspecialchars($tagSlug, ENT_QUOTES) ?>"><?php endif; ?>
    ...
    <?php if ($q !== ''): ?>
      <a class="btn" href="<?= APP_BASE ?>/index.php<?= $mine ? '?mine=1' : '' ?>" style="background:#374151">Effacer</a>
    <?php endif; ?>
  </form>
