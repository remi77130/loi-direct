...
/**
 * feed.php — Feeed des projet publiés
...
if ($isUserQuery && $qraw !== '') {
    ...
    if (mb_strlen($needle) >= 2) {
        // IMPORTANT: feed sur users(pseudo) (utf8mb4_general_ci) pour perf
...
$joins
WHERE $where
ORDER BY p.published_at DESC /*-- tri par date de publication -*/
LIMIT ? OFFSET ?";
...
<title>Tchat direct</title>
  ...
  <link rel="stylesheet" href="<?= APP_BASE ?>/styles/feed.css">
...
<main class="wrap">
  ...
  ?>
    <div class="meta meta--pad">
      ...
      — <a href="<?= APP_BASE ?>/feed.php<?= $noTagQuery ? ('?'.http_build_query($noTagQuery)) : '' ?>" class="link">Effacer</a>
    </div>
  <?php endif; ?>
  ...
  <?php else: ?>
    ...
    <hr />
      ...
      <?php if ($totalPages > 1): ?>
        <div class="pager">
          <?php if ($page > 1): ?>
            <a href="<?= APP_BASE ?>/feed.php?<?= http_build_query($qsForPager + ['page'=>$page-1]) ?>">&laquo; Précédent</a>
          <?php endif; ?>

          <span class="pager-meta">Page <?= $page; ?> / <?= $totalPages; ?></span>

          <?php if ($page < $totalPages): ?>
            <a href="<?= APP_BASE ?>/feed.php?<?= http_build_query($qsForPager + ['page'=>$page+1]) ?>">Suivant &raquo;</a>
          <?php endif; ?>
        </div>
      <?php endif; ?>
